...
if command -v pacman >/dev/null 2>&1; then
    PM=pacman
    PKGS="git make python python-gobject gtk4 libadwaita adwaita-icon-theme libusb pipewire wireplumber alsa-utils libpulse swh-plugins"
    INSTALL_CMD="pacman -S --needed --noconfirm $PKGS"
elif command -v apt-get >/dev/null 2>&1; then
    PM=apt
    PKGS="git make python3 python3-gi gir1.2-gtk-4.0 gir1.2-adw-1 libadwaita-1-0 adwaita-icon-theme libusb-1.0-0 pipewire pipewire-bin wireplumber alsa-utils pulseaudio-utils swh-plugins"
    INSTALL_CMD="apt-get update && apt-get install -y $PKGS"
elif command -v dnf >/dev/null 2>&1; then
    PM=dnf
    PKGS="git make python3 python3-gobject gtk4 libadwaita adwaita-icon-theme libusb1 pipewire pipewire-utils wireplumber alsa-utils pulseaudio-utils ladspa-swh-plugins"
    INSTALL_CMD="dnf install -y $PKGS"
elif command -v zypper >/dev/null 2>&1; then
    PM=zypper
    PKGS="git make python3 python3-gobject gtk4 typelib-1_0-Gtk-4_0 libadwaita-1-0 adwaita-icon-theme typelib-1_0-Adw-1 libusb-1_0-0 pipewire pipewire-tools wireplumber alsa-utils pulseaudio-utils ladspa-swh-plugins"
    INSTALL_CMD="zypper install -y $PKGS"
elif command -v xbps-install >/dev/null 2>&1; then
    PM=xbps
    PKGS="git make python3 python3-gobject gtk4 libadwaita adwaita-icon-theme libusb pipewire wireplumber alsa-utils pulseaudio-utils swh-plugins"
